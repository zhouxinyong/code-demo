...
function imgLazyLoad (imgBoxSelector, isFadeIn, listBoxSelector, fadeInClass) {
  let imgBoxArray = [...document.querySelectorAll(imgBoxSelector)]
  let isScroll = false // 让滚动事件执行的少一点
  // let removeEventTime = null

  // ranking排行榜页面用到的列项容器
  let listBox = isFadeIn !== undefined ? document.querySelectorAll(listBoxSelector) : null
  ...
  let imgShow = function () {
    ...
    // 如果这个图片容器已经插入了图片则退出此次操作
      ...
      let showEleOffsetTop = listBox === null ? imgBox.offsetTop : listBox[key].offsetTop
      ...
      if (scrollTop + window.innerHeight >= showEleOffsetTop - 50) {
        let coverImgUrl = imgBox.getAttribute('data-img')
        let coverImgHtml = `<div class="cover-img"
    ...
    // 图片加载完毕后清除滚动事件
    let imgDisplayComplete = imgBoxArray.every((item) => {
