...
** slideActiveElm 焦点图下方的圆点 Selector
** activeClass 圆点active时添加的class String
*/
export default function slideTouch (slideSelector, slideActiveElm, activeClass) {
  console.log(slideSelector)
  let slide = document.querySelector(slideSelector)
  console.log(slide)
  // 同一个元素只能执行一次
  ...
  let slideActive = document.querySelectorAll(slideActiveElm) // 焦点图下方的点
  let slideActiveIndex = 0
  let touchStartX = 0 // 按下屏幕后记录X坐标
  let diffX = 0 // 松开屏幕时的X坐标 - touchStartX 的差值
  let slideIndex = 1 // 当前拖动到第几张图片
  let slideNum = slide.children.length // 有几张焦点图
  let translateX = -10 // 左右位移的值
  let rootValue = 37.5 // root font-size: 37.5px default
  let intervalId = null
  ...
  slide.addEventListener('touchmove', function (event) {
    ...
    slide.style.transform = 'translateX(' + (translateX + diffX / rootValue) + 'rem)'
  })

  // 图片切换执行的内容
  function slideChangeHandle () {
    slide.style.transition = 'all 0.2s ease'
    // 焦点图位移，当前第几张图片 * 视口宽度为10rem
    translateX = -slideIndex * 10
    slide.style.transform = 'translateX(' + translateX + 'rem)'
